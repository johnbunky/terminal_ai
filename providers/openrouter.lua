...
local function tmpfile(suffix)
    ...
    return TMPDIR .. SEP .. "ai_openrouter_" .. stamp .. (suffix or ".tmp")
end
...
-- ── build messages (identical logic) ──────────────────────────────────────────
...
function M.call(prompt, opts)
    ...
    if not api_key or api_key == "" then
        return nil, "OPENROUTER_API_KEY is not set"
    end
    ...
    local referer = os.getenv("OPENROUTER_REFERER") or "http://localhost"
    local title   = os.getenv("OPENROUTER_TITLE")   or "ai-cli"
